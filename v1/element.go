...
func (e *Element) Type(text string, delay time.Duration) error {
	...
	for _, c := range text {
		if control.isKey(c) {
			if err = e.frame.Session().Input.Press(control.keyDefinitions[c]); err != nil {
				return err
			}
		} else {
...
func (e Element) ClickWith(button input.MouseButton, delayToRelease time.Duration) error {
	...
	cancel := e.frame.session.onBindingCalled(control.bindClick, func(p string) {
		select {
		case clickValue <- p:
		default:
		}
...
func (e Element) GetAttribute(attr string) (string, error) {
	...
	return control.primitiveRemoteObject(*v).String()
}
...
func (e *Element) IsChecked() (bool, error) {
	...
	return control.primitiveRemoteObject(*v).Bool()
}
...
func (e Element) stringArray(v *runtime.RemoteObject) ([]string, error) {
	...
	for _, d := range descriptor {
		...
		val, err1 := control.primitiveRemoteObject(*d.Value).String()
		if err1 != nil {
			return nil, err1
		}
