...
func DefaultDial(context context.Context, url string) (*Transport, error) {
	return Dial(context, DefaultDialer, url)
}

func Dial(parent context.Context, dialer websocket.Dialer, url string) (*Transport, error) {
...
func (t *Transport) Send(request *Request) ResponseFuture {
	...
	request.ID = seq
	log.Println("send -> " + request.String())
	t.mutex.Unlock()
...
func (t *Transport) read() error {
	...
	log.Println("recv <- " + response.String())
